...
const obtenerInformacion = async () => {
  ...
  try{
    const producto = await clienteServices.detalleProducto(id);
    ...
    if(producto.nombre && producto.precio && producto.descripcion && producto.imagen){
      ...
      clienteServices.listaProductos().then(data => {
        data.forEach(({nombre, precio, descripcion, imagen, id, categoria}) => {
          ...
          if(categoria === "Combos" && categoriaSolicitada === "Combos" && idProductoVisto != id){
          ...
          }else if(categoria === "Hamburguesas" && categoriaSolicitada === "Hamburguesas" && idProductoVisto != id){
            const nuevoProducto = mostrarProductosRelacionados(nombre, precio, descripcion, imagen, id, categoria);
            productosSimilares.appendChild(nuevoProducto);
          }else if(categoria === "Pizzas" && categoriaSolicitada === "Pizzas" && idProductoVisto != id){
            const nuevoProducto = mostrarProductosRelacionados(nombre, precio, descripcion, imagen, id, categoria);
            productosSimilares.appendChild(nuevoProducto);
          }else if(categoria === "Bebidas" && categoriaSolicitada === "Bebidas" && idProductoVisto != id){
            const nuevoProducto = mostrarProductosRelacionados(nombre, precio, descripcion, imagen, id, categoria);
            productosSimilares.appendChild(nuevoProducto);
          }
